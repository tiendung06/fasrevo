...
const Products = () => {
  ...
  const init = (pageNumber = 1) => {
    axios
      .get(searchItem.getSearchByCdid(pageNumber, 8, 0))
      .then((resp) => {
    ...
    <Main heading="Thời trang nữ | Mũ">
      <Section>
        <div className="flex items-center pt-10 pb-5 text-xs md:text-sm">
          ...
          <Link href="/women/accessory/hat">
            <a className="font-bold pl-2 lg:pl-3 text-primary_red">Mũ</a>
          </Link>
        </div>
        <SectionHeading>Mũ</SectionHeading>
        <List
