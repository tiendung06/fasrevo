...
const Shorts = () => {
  ...
  const init = (pageNumber = 1) => {
    axios
      .get(searchItem.getSearchByCdid(pageNumber, 7, 0))
      .then((resp) => {
  ...
  return (
    <Main heading="Thời trang nữ | Quần shorts">
      <Section>
        <div className="flex items-center pt-10 pb-5 text-xs md:text-sm">
          ...
          <Link href="/women/bottom/shorts">
            <a className="font-bold pl-2 lg:pl-3 text-primary_red">
              Quần shorts
            </a>
          </Link>
        </div>
        <SectionHeading>Quần shorts</SectionHeading>
        <List
