...
const Shirts = () => {
  ...
  const init = (pageNumber = 1) => {
    axios
      .get(searchItem.getSearchByCdid(pageNumber, 3, 0))
      .then((resp) => {
        setShirts(resp.data.products);
  ...
  return (
    <Main heading="Thời trang nữ | Áo sơ mi">
      <Section>
        <div className="flex items-center pt-10 pb-5 text-xs md:text-sm">
          ...
          <Link href="/women/top/shirts">
            <a className="font-bold pl-2 lg:pl-3 text-primary_red">Áo sơ mi</a>
          </Link>
        </div>
        <SectionHeading>Áo sơ mi</SectionHeading>
        <List
          productItems={shirts}
